...
public final class ListUtilTest extends BaseUnitTest {
	...
	private final String str1 = "t1";
	private final String str2 = "t2";
	...
	public void asList_test() {
		SoftAssertUnitTest.assertTrueNow(
				strList.size() == 2,
				"Result list size of asList is [" + strList.size()
						+ "] and should be 2. Result list [" + Arrays.toString(strList.toArray()) + "]",
				"Verify that result list size of asList is [2]");
	...
	public void asList_with_startIndex_test() throws AutomationUnitTestException {
		...
		SoftAssertUnitTest.assertTrueNow(true,
				"Result of asList should not be null",
		...
		SoftAssertUnitTest.assertTrueNow(resultList.size() == resultListSize,
				"Result list size of asList with input array ["
						+ Arrays.toString(arr) + "] and index [" + startIndex
						+ "] should be [" + resultListSize + "]. result list ["
						+ Arrays.toString(resultList.toArray())	+ "]",
				"Verify that result of asList with input array [" + Arrays.toString(arr)
						+ "] and index [" + startIndex + "] is [" + resultListSize
		...
		for (int i=0; i < resultListSize; i++) {
			SoftAssertUnitTest.assertTrueNow(resultList.get(i).equals(arr[startIndex + i]),
					"Element [" + resultList.get(i) + "] in result list index [" + i
							+ "] is different from element [" + arr[startIndex + i]
							+ "] in input array index [" + (startIndex + i) + "]",
					"Verify that element [" + resultList.get(i) + "] in result list index [" + i
	...
	public void asList_with_startIndex_outOfBound_negative_test() throws AutomationUnitTestException {
		...
		logger.info(
				"Run ListUtil.asList with input array [" + Arrays.toString(arr)
		...
		SoftAssertUnitTest.assertTrueNow(
				...
				"Verify that result of asList with input array [" + Arrays.toString(arr)
						+ "] and index [" + startIndex + "] is empty list");
	}
	...
	public void getContainList_test() {
		...
		SoftAssertUnitTest.assertTrueNow(
				...
				"Verify that result of getContainList with input list ["
						+ Arrays.toString(inputStrList.toArray())
						+ "] and input contain list [" + Arrays.toString(containList.toArray())
		...
		for (String contain : resultContainList) {
			SoftAssertUnitTest.assertTrueNow(
					inputStrList.contains(contain),
					"Input list [" + Arrays.toString(inputStrList.toArray())
							+ "] not contains [" + contain
							+ "] element in list [" + Arrays.toString(containList.toArray()) + "]",
					"Verify that input list [" + Arrays.toString(inputStrList.toArray())
							+ "] contains [" + contain
							+ "] element in list [" + Arrays.toString(containList.toArray()) + "]");
		}
	}
	...
	public void getContainList_not_contain_negative_test() {
		final List<String> inputStrList = List.of("not_contain");
	...
	public void getMultilineStringFromList_test() {
		...
		SoftAssertUnitTest.assertTrueNow(
				expectedMultilineResult.equals(multiLineStrResult),
				"getMultilineStringFromList result is [" + multiLineStrResult
						+ "] and should be [" + expectedMultilineResult + "]",
	...
	private void verifyElementInListInIndexEqualToString(List<String> list, int index, String str) {
		SoftAssertUnitTest.assertTrueNow(
				list.get(index).equals(str),
				"Element [" + list.get(index) + "] in result list in index ["
						+ index + "] should be [" + str + "]",
				"Verify that element in index [" + index
						+ "] in result list is equal to [" + str + "]");
	}
