...
public class StringUtilTest extends BaseUnitTest {
	...
	public void getContainList_test() {
		...
		SoftAssertUnitTest.assertTrueNow(result != null, "Result of getContainList should not be null",
				"Verify that resulf of getContainList is not null");

		final int resultListSize = 2;

		SoftAssertUnitTest.assertTrueNow(result.size() == resultListSize,
	...
	public void isExpect_test(ExpectObject expect, boolean expectedResult) {
		SoftAssertUnitTest.assertTrueNow(StringUtil.isExpect(input, Arrays.asList(expect)) == expectedResult,
	...
	public void replace_test() {
		...
		try {
			...
			SoftAssertUnitTest.failNow("replace should not return exception [" + e.getMessage() + "]");
		}
